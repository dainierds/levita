...
const ServicePlanner: React.FC<ServicePlannerProps> = ({ tier, users, role = 'ADMIN' }) => {
  ...
  const handleCreatePlan = async () => {
    ...
    const defaultTitle = t('planner.new_service') || 'Nuevo Servicio';
    ...
    if (draft) {
      ...
      await savePlan({ ...draft, isRosterDraft: false, title: defaultTitle });
  ...
  if (visiblePlans.length === 0 && !loading) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-slate-500 space-y-4">
        <p>{t('planner.no_plans') || "No hay planes de servicio creados."}</p>
        {!readOnly && (
          <button
          ...
          >
            <Plus size={20} /> {t('planner.create_first') || "Crear Primer Plan"}
          </button>
        )}
      </div>
    );
  ...
  return (
    <div className="p-4 md:p-8 md:pt-24 max-w-full mx-auto space-y-8 pb-32">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-3xl font-bold text-slate-800">{t('planner.title') || "Orden de Cultos"}</h2>
          <p className="text-slate-500">{readOnly ? (t('planner.read_only') || 'Vista de solo lectura.') : (t('planner.subtitle') || 'Diseña el flujo del servicio.')}</p>
        </div>
        ...
        {selectedPlan && !readOnly && (
          <button
            onClick={async () => {
              if (confirm(t('planner.confirm_delete') || '¿Estás seguro de eliminar este plan?')) {
            ...
            title={t('planner.delete_current') || "Eliminar Plan Actual"}
          >
            <Trash2 size={20} />
          </button>
        )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100">
            <div className="mb-4">
              <label className="block text-xs font-bold text-slate-500 mb-2 uppercase">{t('planner.service_name') || "Nombre del Servicio"}</label>
              <input
                type="text"
                disabled={readOnly}
                ...
                placeholder={t('planner.service_name_placeholder') || "Ej. Culto Dominical"}
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-500 mb-2 uppercase">{t('common.date') || "Fecha"}</label>
              <input
                type="date"
                disabled={readOnly}
          ...
          {tier !== 'BASIC' && !readOnly && (
            <div className="card-soft p-6 flex items-center justify-between">
              <div>
                <h3 className="font-bold text-slate-800">{t('planner.status_official') || "Estado Oficial"}</h3>
                <p className="text-xs text-slate-400">{t('planner.publish_app') || "Publicar en App"}</p>
              </div>
              <button
        ...
        <div className="lg:col-span-2 space-y-4">
            ...
            >
              ...
              <div className="flex items-start gap-4 pl-6">
                ...
                <div className="min-w-[4rem] text-center pt-1">
                  ...
                  <span className="text-xs text-slate-400">{item.durationMinutes} {t('common.min') || "min"}</span>
                </div>
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    {!readOnly && (
                      <button
                        onClick={async () => {
                          if (!selectedPlan) return;
                          if (confirm(t('common.confirm_delete') || '¿Eliminar este elemento?')) {
                      ...
                      <div className="flex gap-3">
                        ...
                        {item.youtubeLink && (
                          <a href={item.youtubeLink} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-xs text-red-500 font-medium hover:underline">
                            <PlayCircle size={14} /> {t('planner.see_reference') || "Ver Referencia"}
                          </a>
                        )}
                      </div>
                  ...
                  {item.type === 'PREACHING' && (
                    <div className="mt-4 bg-indigo-50/50 rounded-2xl p-4 border border-indigo-100">
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-xs font-bold text-indigo-400 uppercase">{t('planner.notes_outline') || "Notas & Bosquejo"}</span>
                        {!readOnly && (
                          <div className="text-xs text-slate-400 italic">
                            {t('planner.use_sermons_ia') || "Usa la sección 'Sermones' para generar contenido con IA."}
                          </div>
                        )}
                      </div>
                      <div className="text-sm text-slate-600 whitespace-pre-wrap font-serif">
                        {item.notes || <span className="text-slate-400 italic">{t('planner.no_notes') || "No hay notas adjuntas."}</span>}
                      </div>
                    </div>
                  )}
                </div>
          ...
          {!readOnly && (
            ...
            >
              <Plus size={20} /> {t('planner.add_item') || "Añadir Ítem"}
            </button>
          )}
        </div>
      </div>
          ...
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-[2rem] w-full max-w-2xl p-8 shadow-2xl animate-in fade-in zoom-in duration-200 max-h-[90vh] overflow-y-auto custom-scrollbar">
              <div className="flex justify-between items-center mb-6">
                <div>
                  <h3 className="text-2xl font-bold text-slate-800">{t('planner.add_element_title') || "Añadir Elemento"}</h3>
                  <p className="text-slate-500 text-sm">{t('planner.add_element_subtitle') || "Selecciona una plantilla o crea uno personalizado."}</p>
                </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 mb-8">
                {[
                  { label: t('planner.preset_welcome') || 'Bienvenida', duration: 5, icon: Hand, type: 'GENERIC', color: 'text-orange-500 bg-orange-50' },
                  { label: t('planner.preset_worship') || 'Alabanza', duration: 20, icon: Music, type: 'WORSHIP', color: 'text-pink-500 bg-pink-50' },
                  { label: t('planner.preset_prayer') || 'Oración', duration: 5, icon: UserIcon, type: 'GENERIC', color: 'text-blue-500 bg-blue-50' },
                  { label: t('planner.preset_reading') || 'Lectura', duration: 5, icon: BookOpen, type: 'GENERIC', color: 'text-indigo-500 bg-indigo-50' },
                  { label: t('planner.preset_preaching') || 'Predicación', duration: 40, icon: Mic2, type: 'PREACHING', color: 'text-violet-500 bg-violet-50' },
                  { label: t('planner.preset_offering') || 'Ofrenda', duration: 10, icon: DollarSign, type: 'GENERIC', color: 'text-green-500 bg-green-50' },
                  { label: t('planner.preset_announcements') || 'Anuncios', duration: 5, icon: Megaphone, type: 'GENERIC', color: 'text-red-500 bg-red-50' },
                  { label: t('planner.preset_communion') || 'Cena Señor', duration: 15, icon: Gift, type: 'GENERIC', color: 'text-amber-500 bg-amber-50' },
                  { label: t('planner.preset_special') || 'Especial', duration: 5, icon: Star, type: 'GENERIC', color: 'text-yellow-500 bg-yellow-50' },
                  { label: t('planner.preset_testimonies') || 'Testimonios', duration: 10, icon: MessageCircle, type: 'GENERIC', color: 'text-teal-500 bg-teal-50' },
                  { label: t('planner.preset_sick') || 'Enfermos', duration: 10, icon: Heart, type: 'GENERIC', color: 'text-rose-500 bg-rose-50' },
                  { label: t('planner.preset_custom') || 'Personalizado', duration: 5, icon: PenTool, type: 'GENERIC', color: 'text-slate-500 bg-slate-100', isCustom: true },
                ].map((preset) => (
                  <button
                    key={preset.label}
                    onClick={() => {
                  ...
                  >
                    ...
                    <span className="text-[10px] text-slate-400">{preset.duration} {t('common.min') || "min"}</span>
                  </button>
                ))}
              </div>
              ...
              <div className="bg-slate-50 rounded-3xl p-6 border border-slate-100 space-y-4">
                <h4 className="text-sm font-bold text-slate-500 uppercase mb-2">{t('planner.item_details') || "Detalles del Elemento"}</h4>

                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1">{t('common.title') || "Título"}</label>
                  <input
                    type="text"
                    value={newItem.title}
                    ...
                    placeholder={t('planner.item_title_placeholder') || "Escribe el nombre del elemento..."}
                  />
                </div>

                <div className="flex gap-4">
                  <div className="flex-1">
                    <label className="block text-xs font-bold text-slate-500 mb-1">{t('planner.duration_min') || "Duración (minutos)"}</label>
                    <input
                      type="number"
                  ...
                  {newItem.type === 'WORSHIP' && (
                    <div className="flex-1 animate-in slide-in-from-left duration-300">
                      <label className="block text-xs font-bold text-slate-500 mb-1">{t('planner.key') || "Tonalidad (Key)"}</label>
                      <input
                        type="text"
                        value={newItem.key || ''}
                        onChange={(e) => setNewItem({ ...newItem, key: e.target.value })}
                        className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all font-medium"
                        placeholder={t('planner.key_placeholder') || "Ej. G, Dm, Bb..."}
                      />
                    </div>
                  )}
                </div>
                ...
                <div className="pt-4">
                  ...
                  >
                    <Plus size={20} /> {t('planner.add_to_order') || "Añadir al Orden"}
                  </button>
                </div>
              </div>
