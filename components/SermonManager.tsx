...
const SermonManager: React.FC = () => {
    ...
    const handleSave = async () => {
        ...
        setSelectedSermon(null); // Return to list or keep editing? Let's keep editing but update ID if needed.
        // Actually simpler to just refresh list, but for UX let's go back to list or stay.
        // If it was new, it will now appear in the list.
    };
    ...
    return (
        <div className="p-4 md:p-8 space-y-8 max-w-full mx-auto pb-20 pt-24 md:pt-32">
            <header className="flex flex-col items-start gap-4">
                <div>
                    <h2 className="text-3xl font-bold text-slate-800">Gestor de Sermones</h2>
                    <p className="text-slate-500">Crea, organiza y genera bosquejos con IA.</p>
                </div>
                {!selectedSermon && (
                    ...
                    {sermons.map(sermon => (
                        ...
                        >
                            ...
                            <p className="text-xs text-slate-500 flex items-center gap-2">
                                <BookOpen size={12} /> {sermon.passage || 'Sin pasaje'} • {sermon.date}
                            </p>
                        </div>
                    ))}
                </div>
                ...
                <div className={`lg:col-span-2 ${selectedSermon ? 'block' : 'hidden lg:block'}`}>
                    {selectedSermon ? (
                        <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden flex flex-col h-[calc(100vh-200px)]">
                            ...
                            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                                ...
                                <div className="flex gap-2">
                                    <button onClick={handleSave} className="p-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors" title="Guardar">
                                        <Save size={20} />
                                    </button>
                                    {selectedSermon.id && (
                                        <button
                                            onClick={async () => {
                                                if (confirm("¿Borrar sermón?")) {
                            ...
                            <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                                ...
                                <div className="bg-indigo-50 p-4 rounded-2xl border border-indigo-100">
                                    <div className="flex flex-col md:flex-row gap-4 items-end">
                                        <div className="flex-1 w-full">
                                            <label className="block text-xs font-bold text-indigo-400 uppercase mb-1">Pasaje Bíblico</label>
                                            <input
                                                type="text"
                                        ...
                                        <div className="w-full md:w-40">
                                            <label className="block text-xs font-bold text-indigo-400 uppercase mb-1">Tono</label>
                                            <select
                                                value={aiParams.tone}
                                ...
                                <div className="relative">
                                    <textarea
                                        ...
                                        placeholder="Escribe tu bosquejo aquí o genéralo con IA..."
                        ...
                        <div className="h-full flex flex-col items-center justify-center text-slate-400 bg-slate-50 rounded-[2.5rem] border border-dashed border-slate-200 p-8 text-center">
                            ...
                            <h3 className="text-xl font-bold text-slate-600 mb-2">Selecciona o crea un sermón</h3>
                            <p className="max-w-xs mx-auto">Usa la inteligencia artificial para generar bosquejos bíblicos en segundos.</p>
                        </div>
                    )}
                </div>
            </div>
