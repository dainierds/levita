        ...
        <>
            ...
            <div className={`${drawerClasses} ${isOpen ? 'shadow-2xl' : ''} lg:z-0 flex flex-col h-full p-6`}>
                <div className="flex flex-col items-center justify-center pt-8 pb-8">
                    <button onClick={onClose} className="absolute top-6 right-6 lg:hidden p-3 rounded-full text-gray-500 bg-neu-base dark:bg-neu-base-dark shadow-neu dark:shadow-neu-dark active:shadow-neu-pressed dark:active:shadow-neu-dark-pressed transition-all"><X size={20} /></button>
                ...
                <nav className="flex-1 space-y-4 overflow-y-auto no-scrollbar py-2">
                    {navItems.map((item) => {
                        ...
                        const baseClasses = "w-full flex items-center space-x-4 px-6 py-4 rounded-2xl transition-all duration-300 bg-neu-base dark:bg-neu-base-dark";
                        // State specific classes
                        const activeClasses = "text-brand-500 shadow-neu-pressed dark:shadow-neu-dark-pressed font-bold transform scale-[0.98]";
                        const inactiveClasses = "text-gray-500 dark:text-gray-400 shadow-neu dark:shadow-neu-dark hover:-translate-y-1";

                        return (
                            <button key={item.id} onClick={() => handleNavClick(item.id)} className={`${baseClasses} ${isActive ? activeClasses : inactiveClasses}`}>
