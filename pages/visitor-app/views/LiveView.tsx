    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-[calc(100vh-10rem)]">
      ...
      <div className="lg:col-span-2 flex flex-col space-y-6">
        ...
        <div className="p-2 bg-neu-base dark:bg-neu-base-dark rounded-[2.5rem] shadow-neu dark:shadow-neu-dark">
          <div className="relative aspect-video bg-gray-900 rounded-[2rem] overflow-hidden shadow-inner">
            ...
            <iframe
              width="100%"
              height="100%"
              src="https://www.youtube.com/embed/live_stream?channel=UCrG2l_b2s_Bv-K37-Gq-Y2w&enablejsapi=1&autoplay=1&mute=1&playsinline=1&origin=http://localhost:3000"
        ...
        <div className="p-6 rounded-3xl bg-neu-base dark:bg-neu-base-dark shadow-neu dark:shadow-neu-dark flex flex-col sm:flex-row items-center justify-between">
          <div className="mb-4 sm:mb-0">
            <h1 className="text-2xl font-bold text-gray-700 dark:text-gray-200">Transmisión en Vivo</h1>
            <p className="text-brand-500 font-medium mt-1">Iglesia Adventista del Séptimo Día</p>
          </div>
      ...
      <div className="lg:col-span-1 bg-neu-base dark:bg-neu-base-dark rounded-[2.5rem] shadow-neu dark:shadow-neu-dark p-6 flex flex-col overflow-hidden h-96 lg:h-auto">
        ...
        <div className="flex-1 overflow-y-auto space-y-4 pr-2 custom-scrollbar">
          ...
          {messages.map((msg) => (
            <div key={msg.id} className={`flex flex-col ${msg.user === 'Visitante' ? 'items-end' : 'items-start'}`}>
              <span className="text-[10px] font-bold text-gray-400 mb-1 px-1 flex items-center gap-1">
                {msg.user !== 'Visitante' && <User size={10} />}
                {msg.user}
              </span>
              <div className={`px-4 py-3 max-w-[85%] text-sm font-medium ${msg.user === 'Visitante'
